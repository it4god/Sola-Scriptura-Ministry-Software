...
Begin VB.Form frmKHS 
   ...
   Begin VB.TextBox TxtIndentity 
      ...
      Width           =   7987
   End
   Begin MSDataGridLib.DataGrid DataGridKHS 
      Height          =   6015
      Left            =   3360
      TabIndex        =   10
      Top             =   1320
   ...
   Begin VB.Image imgBg 
      Height          =   8055
      Left            =   2760
      Picture         =   "FrmKHS.frx":6D30D
      Stretch         =   -1  'True
      Top             =   240
      Width           =   12480
   End
End
Attribute VB_Name = "frmKHS"
...
Private Sub Form_Load()
    ...
    Set rsKHS = myitem.GetKHS(Periode, Trim(CurrentMahasiswa.NIM))
    Set DataGridKHS.DataSource = rsKHS
    DataGridKHS.Columns(0).Width = 1000
    DataGridKHS.Columns(1).Width = 3000
    DataGridKHS.Columns(2).Width = 800
    DataGridKHS.Columns(3).Width = 800
    DataGridKHS.Columns(4).Width = 800
    DataGridKHS.Columns(5).Width = 800
    DataGridKHS.Columns(6).Width = 1000
...
Dim KHSReport As DataReportKHS
Set KHSReport = New DataReportKHS
Set KHSReport.DataSource = rsKHS

Dim myreportlabel(1 To 9) As RptLabel
    Dim x As Integer
    For x = 1 To 9
        Set myreportlabel(x) = KHSReport.Sections(1).Controls.Item(x)
    Next x
...
myreportlabel(5).Caption = Trim(strperiode)
...
Dim myreportlabel_2(1 To 12) As RptLabel
    For x = 1 To 12
        Set myreportlabel_2(x) = KHSReport.Sections(5).Controls.Item(x)
    Next x


Dim JumlahSKS As Double
Dim JumlahNilai As Double
Dim IP As Double
Dim SKSKumulatif As Double
Dim NilaiKumulatif As Double
Dim IPKumulatif As Double

Dim myKHS As ADODB.Recordset
Set myitem = New ClsItemServer
Set myKHS = myitem.GetKHS(Periode, Trim(CurrentMahasiswa.NIM))

JumlahSKS = 0
JumlahNilai = 0
IP = 0
If myKHS.RecordCount > 0 Then
    myKHS.MoveFirst
    While Not myKHS.EOF
        If Trim(myKHS.Fields("Grade")) <> "Pd" Then
            JumlahSKS = JumlahSKS + CDbl(myKHS.Fields("SKS"))
            JumlahNilai = JumlahNilai + CDbl(myKHS.Fields("Nilai"))
...
Dim myKHSKumulatif As ADODB.Recordset
Set myitem = New ClsItemServer
Set myKHSKumulatif = myitem.GetKHSKumulatif(Periode, Trim(CurrentMahasiswa.NIM))

SKSKumulatif = 0
NilaiKumulatif = 0
IPKumulatif = 0

If myKHSKumulatif.RecordCount > 0 Then
    myKHSKumulatif.MoveFirst
    While Not myKHSKumulatif.EOF
        If Trim(myKHSKumulatif.Fields("Grade")) <> "Pd" Then
            SKSKumulatif = SKSKumulatif + CDbl(myKHSKumulatif.Fields("SKS"))
            NilaiKumulatif = NilaiKumulatif + CDbl(myKHSKumulatif.Fields("Nilai"))
            
        End If
        myKHSKumulatif.MoveNext
...
End If
b = InStr(1, CStr(IP), ",", vbTextCompare)
If b = 0 Then
    myIP = CStr(IP) & ",0"
Else
    myIP = Mid(CStr(IP), 1, b + 2)
End If
...
myreportlabel_2(9).Caption = Trim(myIP)
